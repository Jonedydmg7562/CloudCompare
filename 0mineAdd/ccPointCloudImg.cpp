...
ccPointCloudImg::ccPointCloudImg(QString name) throw()
...
ccPointCloudImg* ccPointCloudImg::From(CCLib::GenericCloud* cloud, const ccGenericPointCloud* sourceCloud/*=0*/)
{
	ccPointCloudImg* pc = new ccPointCloudImg("Cloud");

	unsigned n = cloud->size();
	if (n == 0)
	{
		ccLog::Warning("[ccPointCloudImg::From] Input cloud is empty!");
	}
	else
	{
		if (!pc->reserveThePointsTable(n))
		{
			ccLog::Error("[ccPointCloudImg::From] Not enough memory to duplicate cloud!");
			delete pc;
			pc = nullptr;
		}
		else
...
ccPointCloudImg* ccPointCloudImg::From(const CCLib::GenericIndexedCloud* cloud, const ccGenericPointCloud* sourceCloud/*=0*/)
{
	ccPointCloudImg* pc = new ccPointCloudImg("Cloud");
	...
	else
	{
		if (!pc->reserveThePointsTable(n))
		{
			ccLog::Error("[ccPointCloudImg] Not enough memory to duplicate cloud!");
			delete pc;
			pc = nullptr;
		}
		else
...
void UpdateGridIndexes(const std::vector<int>& newIndexMap, std::vector<ccPointCloudImg::Grid::Shared>& grids)
{
	for (ccPointCloudImg::Grid::Shared& scanGrid : grids)
	{
...
ccPointCloudImg* ccPointCloudImg::partialClone(const CCLib::ReferenceCloud* selection, int* warnings/*=0*/) const
{
	if (warnings)
	{
		*warnings = 0;
	...
	{
		ccLog::Error("[ccPointCloudImg::partialClone] Invalid parameters");
		return nullptr;
	}

	ccPointCloudImg* result = new ccPointCloudImg(getName() + QString(".extract"));
	...
	if (n)
	{
		...
		{
			ccLog::Error("[ccPointCloudImg::partialClone] Not enough memory to duplicate cloud!");
			delete result;
			return nullptr;
		}
		...
		if (hasColors())
		{
			...
			{
				ccLog::Warning("[ccPointCloudImg::partialClone] Not enough memory to copy RGB colors!");
				if (warnings)
		...
		if (hasNormals())
		{
			...
			{
				ccLog::Warning("[ccPointCloudImg::partialClone] Not enough memory to copy normals!");
				if (warnings)
		...
		if (hasFWF())
		{
			if (result->reserveTheFWFTable())
			{
				...
				{
					ccLog::Warning("[ccPointCloudImg::partialClone] Not enough memory to copy waveform signals!");
					result->clearFWFData();
					if (warnings)
				...
				if (sf)
				{
					...
					if (sfIdx >= 0) //success
					{
						...
						else
						{
							...
							ccLog::Warning(QString("[ccPointCloudImg::partialClone] Not enough memory to copy scalar field '%1'!").arg(sf->getName()));
							if (warnings)
		...
		if (gridCount() != 0)
		{
			...
			{
				//not enough memory
				ccLog::Warning(QString("[ccPointCloudImg::partialClone] Not enough memory to copy the grid structure(s)"));
			}
		}
...
ccPointCloudImg::~ccPointCloudImg()
{
	clear();
...
void ccPointCloudImg::unalloactePoints()
{
...
void ccPointCloudImg::notifyGeometryUpdate()
{
...
ccGenericPointCloud* ccPointCloudImg::clone(ccGenericPointCloud* destCloud/*=0*/, bool ignoreChildren/*=false*/)
{
	if (destCloud && !destCloud->isA(CC_TYPES::POINT_CLOUD))
	{
		ccLog::Error("[ccPointCloudImg::clone] Invalid destination cloud provided! Not a ccPointCloudImg...");
		return nullptr;
	}

	return cloneThis(static_cast<ccPointCloudImg*>(destCloud), ignoreChildren);
}

ccPointCloudImg* ccPointCloudImg::cloneThis(ccPointCloudImg* destCloud/*=0*/, bool ignoreChildren/*=false*/)
{
	ccPointCloudImg* result = destCloud ? destCloud : new ccPointCloudImg();
...
const ccPointCloudImg& ccPointCloudImg::operator +=(ccPointCloudImg* addedCloud)
{
	if (isLocked())
	{
		ccLog::Error("[ccPointCloudImg::fusion] Cloud is locked");
		return *this;
	}
...
const ccPointCloudImg& ccPointCloudImg::append(ccPointCloudImg* addedCloud, unsigned pointCountBefore, bool ignoreChildren/*=false*/)
{
	assert(addedCloud);
	...
	{
		ccLog::Error("[ccPointCloudImg::append] Not enough memory!");
		return *this;
	}
	...
	if (hasColors() || addedCloud->hasColors())
	{
		...
		else //otherwise
		{
			...
			if (!hasColors())
			{
				...
				{
					ccLog::Warning("[ccPointCloudImg::fusion] Not enough memory: failed to allocate colors!");
					showColors(false);
				}
			}
	...
	if (hasNormals() || addedCloud->hasNormals())
	{
		...
		else //otherwise
		{
			...
			if (!hasNormals())
			{
				...
				{
					ccLog::Warning("[ccPointCloudImg::fusion] Not enough memory: failed to allocate normals!");
					showNormals(false);
				}
			}
	...
	if (hasFWF() || addedCloud->hasFWF())
	{
		...
		else //otherwise
		{
			...
			if (!hasFWF())
			{
				...
				{
					success = false;
					ccLog::Warning("[ccPointCloudImg::fusion] Not enough memory: failed to allocate waveforms!");
				}
			}
			else if (fwfData() != addedCloud->fwfData())
			{
				...
				catch (const std::bad_alloc&)
				{
					...
					ccLog::Warning("[ccPointCloudImg::fusion] Not enough memory: failed to merge waveform containers!");
				}
			}
			...
			if (success)
			{
				...
				try
				{
					...
					if (!m_fwfDescriptors.empty())
					{
						...
						for (auto it = addedCloud->m_fwfDescriptors.begin(); it != addedCloud->m_fwfDescriptors.end(); ++it)
						{
							...
							{
								ccLog::Warning("[ccPointCloudImg::fusion] Not enough free FWF descriptor IDs on destination cloud: some waveforms won't be imported!");
								break;
							}
				...
				catch (const std::bad_alloc&)
				{
					success = false;
					clearFWFData();
					ccLog::Warning("[ccPointCloudImg::fusion] Not enough memory: failed to copy waveform descriptors!");
				}
				...
				{
					ccLog::Warning(QString("[ccPointCloudImg::fusion] %1 waveform(s) were lost in the fusion process").arg(lostWaveformCount));
				}
			}
		}
	}
			...
			if (sf)
			{
				...
				else //otherwise we create a new SF
				{
					...
					else
					{
						newSF->release();
						newSF = nullptr;
						ccLog::Warning("[ccPointCloudImg::fusion] Not enough memory: failed to allocate a copy of scalar field '%s'",sf->getName());
					}
				}
			}
		}
		...
		for (size_t i = 0; i < addedCloud->gridCount(); ++i)
		{
			...
			if (otherGrid && otherGrid->validCount != 0) //no need to copy empty grids!
			{
				...
				catch (const std::bad_alloc&)
				{
					//not enough memory
					m_grids.resize(0);
					ccLog::Warning(QString("[ccPointCloudImg::fusion] Not enough memory: failed to copy the grid structure(s) from '%1'").arg(addedCloud->getName()));
					break;
				}
			}
			else
	...
	{
		ccLog::Warning(QString("[ccPointCloudImg::fusion] Grid structure(s) will be dropped as the merged cloud is unstructured"));
		m_grids.resize(0);
	}
	...
	//has the cloud been recentered/rescaled?
	{
		if (addedCloud->isShifted())
		{
					...
					||	(getGlobalShift() - addedCloud->getGlobalShift()).norm2d() > 1.0e-6)
			{
				...
				ccLog::Warning(QString("[ccPointCloudImg::fusion] Global shift/scale information conflict: shift/scale of cloud '%1' will be ignored!").arg(addedCloud->getName()));
			}
		}
	}
	...
	if (!ignoreChildren)
	{
		...
		for (unsigned c = 0; c < childrenCount; ++c)
		{
			...
			if (child->isA(CC_TYPES::MESH)) //mesh --> FIXME: what for the other types of MESH?
			{
				...
				{
					ccLog::Warning(QString("[ccPointCloudImg::fusion] Not enough memory: failed to clone sub mesh %1!").arg(mesh->getName()));
				}
			}
...
void ccPointCloudImg::unallocateNorms()
{
	if (m_normals)
	{
		m_normals->release();
...
void ccPointCloudImg::unallocateColors()
{
	if (m_rgbColors)
	{
		m_rgbColors->release();
...
bool ccPointCloudImg::reserveThePointsTable(unsigned newNumberOfPoints)
{
	try
	{
...
bool ccPointCloudImg::reserveTheRGBTable()
{
	if (m_points.capacity() == 0)
	{
		ccLog::Warning("[ccPointCloudImg::reserveTheRGBTable] Internal error: properties (re)allocation before points allocation is forbidden!");
		return false;
	}
	...
	if (!m_rgbColors->reserveSafe(m_points.capacity()))
	{
		m_rgbColors->release();
		m_rgbColors = nullptr;
		ccLog::Error("[ccPointCloudImg::reserveTheRGBTable] Not enough memory!");
	}
...
bool ccPointCloudImg::resizeTheRGBTable(bool fillWithWhite/*=false*/)
{
	if (m_points.empty())
	{
		ccLog::Warning("[ccPointCloudImg::resizeTheRGBTable] Internal error: properties (re)allocation before points allocation is forbidden!");
		return false;
	}
	...
	if (!m_rgbColors->resizeSafe(m_points.size(), fillWithWhite, &s_white))
	{
		m_rgbColors->release();
		m_rgbColors = nullptr;
		ccLog::Error("[ccPointCloudImg::resizeTheRGBTable] Not enough memory!");
	}
...
bool ccPointCloudImg::reserveTheNormsTable()
{
	if (m_points.capacity() == 0)
	{
		ccLog::Warning("[ccPointCloudImg::reserveTheNormsTable] Internal error: properties (re)allocation before points allocation is forbidden!");
		return false;
	}
	...
	if (!m_normals->reserveSafe(m_points.capacity()))
	{
		...
		ccLog::Error("[ccPointCloudImg::reserveTheNormsTable] Not enough memory!");
	}
...
bool ccPointCloudImg::resizeTheNormsTable()
{
	if (m_points.empty())
	{
		ccLog::Warning("[ccPointCloudImg::resizeTheNormsTable] Internal error: properties (re)allocation before points allocation is forbidden!");
		return false;
	}
	...
	if (!m_normals->resizeSafe(m_points.size(), true, &s_normZero))
	{
		...
		ccLog::Error("[ccPointCloudImg::resizeTheNormsTable] Not enough memory!");
	}
...
bool ccPointCloudImg::compressFWFData()
{
	...
	try
	{
		...
		{
			//nothing to do
			ccLog::Print(QString("[ccPointCloudImg::compressFWFData] Cloud '%1': no need to compress FWF data").arg(getName()));
			return true;
		}
		...
		ccLog::Print(QString("[ccPointCloudImg::compressFWFData] Cloud '%1': FWF data compressed --> %2 / %3 (%4%)").arg(getName()).arg(newIndex).arg(initialCount).arg(100.0 - (newIndex * 100.0) / initialCount, 0, 'f', 1));
	}
	catch (const std::bad_alloc&)
	{
		ccLog::Warning("[ccPointCloudImg::compressFWFData] Not enough memory!");
		return false;
	}
...
bool ccPointCloudImg::reserveTheFWFTable()
{
	if (m_points.capacity() == 0)
	{
		ccLog::Warning("[ccPointCloudImg::reserveTheFWFTable] Internal error: properties (re)allocation before points allocation is forbidden!");
		return false;
	}
	...
	{
		ccLog::Error("[ccPointCloudImg::reserveTheFWFTable] Not enough memory!");
...
bool ccPointCloudImg::hasFWF() const
{
	return		m_fwfData
			&&	!m_fwfData->empty()
...
ccWaveformProxy ccPointCloudImg::waveformProxy(unsigned index) const
{
	...
	if (index < m_fwfWaveforms.size())
	{
		...
		if (m_fwfData && w.dataOffset() + w.byteCount() <= m_fwfData->size())
		{
			...
			{
				WaveformDescriptor& d = const_cast<ccPointCloudImg*>(this)->m_fwfDescriptors[w.descriptorID()]; //DGM: we really want the reference to the element, not a copy as QMap returns in the const case :(
...
bool ccPointCloudImg::resizeTheFWFTable()
{
	if (m_points.capacity() == 0)
	{
		ccLog::Warning("[ccPointCloudImg::resizeTheFWFTable] Internal error: properties (re)allocation before points allocation is forbidden!");
		return false;
	}
	...
	{
		ccLog::Error("[ccPointCloudImg::resizeTheFWFTable] Not enough memory!");
		m_fwfWaveforms.resize(0);
	}
...
bool ccPointCloudImg::reserve(unsigned newNumberOfPoints)
{
	...
	{
		ccLog::Error("[ccPointCloudImg::reserve] Not enough memory!");
		return false;
	}

	//ccLog::Warning(QString("[ccPointCloudImg::reserve] Cloud is %1 and its capacity is '%2'").arg(m_points.capacity() != 0 ? "allocated" : "not allocated").arg(m_points.capacity()));
...
bool ccPointCloudImg::resize(unsigned newNumberOfPoints)
{
	...
	{
		ccLog::Error("[ccPointCloudImg::resize] Not enough memory!");
		return false;
	}
...
void ccPointCloudImg::showSFColorsScale(bool state)
{
	m_sfColorScaleDisplayed = state;
}

bool ccPointCloudImg::sfColorScaleShown() const
{
	return m_sfColorScaleDisplayed;
}

const ccColor::Rgb* ccPointCloudImg::getPointScalarValueColor(unsigned pointIndex) const
{
...
const ccColor::Rgb* ccPointCloudImg::geScalarValueColor(ScalarType d) const
{
	assert(m_currentDisplayedScalarField && m_currentDisplayedScalarField->getColorScale());
...
ScalarType ccPointCloudImg::getPointDisplayedDistance(unsigned pointIndex) const
{
...
const ccColor::Rgb& ccPointCloudImg::getPointColor(unsigned pointIndex) const
{
	assert(hasColors());
...
const CompressedNormType& ccPointCloudImg::getPointNormalIndex(unsigned pointIndex) const
{
...
const CCVector3& ccPointCloudImg::getPointNormal(unsigned pointIndex) const
{
	assert(m_normals && pointIndex < m_normals->currentSize());
...
void ccPointCloudImg::setPointColor(unsigned pointIndex, const ccColor::Rgb& col)
{
	assert(m_rgbColors && pointIndex < m_rgbColors->currentSize());
...
void ccPointCloudImg::setPointNormalIndex(unsigned pointIndex, CompressedNormType norm)
{
	assert(m_normals && pointIndex < m_normals->currentSize());
...
void ccPointCloudImg::setPointNormal(unsigned pointIndex, const CCVector3& N)
{
	setPointNormalIndex(pointIndex, ccNormalVectors::GetNormIndex(N));
}

bool ccPointCloudImg::hasColors() const
{
	return m_rgbColors && m_rgbColors->isAllocated();
}

bool ccPointCloudImg::hasNormals() const
{
	return m_normals && m_normals->isAllocated();
}

bool ccPointCloudImg::hasScalarFields() const
{
	return (getNumberOfScalarFields() > 0);
}

bool ccPointCloudImg::hasDisplayedScalarField() const
{
	return m_currentDisplayedScalarField && m_currentDisplayedScalarField->getColorScale();
}

void ccPointCloudImg::invalidateBoundingBox()
{
...
void ccPointCloudImg::addRGBColor(const ccColor::Rgb& C)
{
...
void ccPointCloudImg::addNorm(const CCVector3& N)
{
	addNormIndex(ccNormalVectors::GetNormIndex(N));
}

void ccPointCloudImg::addNormIndex(CompressedNormType index)
{
	assert(m_normals && m_normals->isAllocated());
	m_normals->addElement(index);
}

void ccPointCloudImg::addNormAtIndex(const PointCoordinateType* N, unsigned index)
{
	assert(m_normals && m_normals->isAllocated());
...
bool ccPointCloudImg::convertNormalToRGB()
{
	if (!hasNormals())
		return false;
	...
	{
		ccLog::Warning("[ccPointCloudImg::convertNormalToRGB] Not enough memory!");
		return false;
	}
...
bool ccPointCloudImg::convertRGBToGreyScale()
{
	if (!hasColors())
	{
		return false;
...
bool ccPointCloudImg::convertNormalToDipDirSFs(ccScalarField* dipSF, ccScalarField* dipDirSF)
{
	if (!dipSF && !dipDirSF)
	{
		assert(false);
	...
	{
		ccLog::Warning("[ccPointCloudImg::convertNormalToDipDirSFs] Not enough memory!");
		return false;
	}
...
void ccPointCloudImg::setNormsTable(NormsIndexesTableType* norms)
{
	if (m_normals == norms)
		return;
...
bool ccPointCloudImg::colorize(float r, float g, float b)
{
...
bool ccPointCloudImg::setRGBColorByBanding(unsigned char dim, double freq)
{
	if (freq == 0 || dim > 2) //X=0, Y=1, Z=2
	{
		ccLog::Warning("[ccPointCloudImg::setRGBColorByBanding] Invalid parameter!");
		return false;
	}
...
bool ccPointCloudImg::setRGBColorByHeight(unsigned char heightDim, ccColorScale::Shared colorScale)
{
	if (!colorScale || heightDim > 2) //X=0, Y=1, Z=2
	{
		ccLog::Error("[ccPointCloudImg::setRGBColorByHeight] Invalid parameter!");
		return false;
	}
...
bool ccPointCloudImg::setRGBColor(const ccColor::Rgb& col)
{
	enableTempColor(false);
...
CCVector3 ccPointCloudImg::computeGravityCenter()
{
	return CCLib::GeometricalAnalysisTools::ComputeGravityCenter(this);
}

void ccPointCloudImg::applyGLTransformation(const ccGLMatrix& trans)
{
	return applyRigidTransformation(trans);
}

void ccPointCloudImg::applyRigidTransformation(const ccGLMatrix& trans)
{
	//transparent call
...
void ccPointCloudImg::translate(const CCVector3& T)
{
...
void ccPointCloudImg::scale(PointCoordinateType fx, PointCoordinateType fy, PointCoordinateType fz, CCVector3 center)
{
...
void ccPointCloudImg::invertNormals()
{
	if (!hasNormals())
		return;
...
void ccPointCloudImg::swapPoints(unsigned firstIndex, unsigned secondIndex)
{
	assert(!isLocked());
...
void ccPointCloudImg::getDrawingParameters(glDrawParams& params) const
{
	//color override
	if (isColorOverriden())
	{
...
void ccPointCloudImg::glChunkVertexPointer(const CC_DRAW_CONTEXT& context, size_t chunkIndex, unsigned decimStep, bool useVBOs)
{
...
void ccPointCloudImg::glChunkNormalPointer(const CC_DRAW_CONTEXT& context, size_t chunkIndex, unsigned decimStep, bool useVBOs)
{
	assert(m_normals);
...
void ccPointCloudImg::glChunkColorPointer(const CC_DRAW_CONTEXT& context, size_t chunkIndex, unsigned decimStep, bool useVBOs)
{
	assert(m_rgbColors);
...
void ccPointCloudImg::glChunkSFPointer(const CC_DRAW_CONTEXT& context, size_t chunkIndex, unsigned decimStep, bool useVBOs)
{
	assert(m_currentDisplayedScalarField);
	assert(sizeof(ColorCompType) == 1);
...
template <class QOpenGLFunctions> void glLODChunkVertexPointer(	ccPointCloudImg* cloud,
...
void ccPointCloudImg::drawMeOnly(CC_DRAW_CONTEXT& context)
{
	if (m_points.empty())
		return;
...
void ccPointCloudImg::addColorRampInfo(CC_DRAW_CONTEXT& context)
{
...
ccPointCloudImg* ccPointCloudImg::filterPointsByScalarValue(ScalarType minVal, ScalarType maxVal, bool outside/*=false*/)
{
...
void ccPointCloudImg::hidePointsByScalarValue(ScalarType minVal, ScalarType maxVal)
{
...
ccGenericPointCloud* ccPointCloudImg::createNewCloudFromVisibilitySelection(bool removeSelectedPoints/*=false*/, VisibilityTableType* visTable/*=nullptr*/, bool silent/*=false*/)
{
	if (!visTable)
	{
		if (!isVisibilityTableInstantiated())
	...
	ccPointCloudImg* result = nullptr;
	{
		...
		if (!rc)
		{
			...
			//ccLog::Warning("[ccPointCloudImg] An error occurred during points selection!");
			return nullptr;
		}
	...
	if (!result)
	{
		ccLog::Warning("[ccPointCloudImg] Failed to generate a subset cloud");
		return nullptr;
	}
...
ccScalarField* ccPointCloudImg::getCurrentDisplayedScalarField() const
{
	return static_cast<ccScalarField*>(getScalarField(m_currentDisplayedScalarFieldIndex));
}

int ccPointCloudImg::getCurrentDisplayedScalarFieldIndex() const
{
	return m_currentDisplayedScalarFieldIndex;
}

void ccPointCloudImg::setCurrentDisplayedScalarField(int index)
{
...
void ccPointCloudImg::deleteScalarField(int index)
{
...
void ccPointCloudImg::deleteAllScalarFields()
{
	//the father does all the work
...
bool ccPointCloudImg::setRGBColorWithCurrentScalarField(bool mixWithExistingColor/*=false*/)
{
	if (!hasDisplayedScalarField())
	{
		ccLog::Warning("[ccPointCloudImg::setColorWithCurrentScalarField] No active scalar field or color scale!");
		return false;
	}
...
QSharedPointer<CCLib::ReferenceCloud> ccPointCloudImg::computeCPSet(	ccGenericPointCloud& otherCloud,
																	...
																	unsigned char octreeLevel/*=0*/)
{
	...
	if (sfIdx < 0)
	{
		ccLog::Warning("[ccPointCloudImg::ComputeCPSet] Not enough memory!");
	...
	if (result < 0)
	{
		ccLog::Warning("[ccPointCloudImg::ComputeCPSet] Closest-point set computation failed!");
		CPSet.clear();
	}
...
bool ccPointCloudImg::interpolateColorsFrom(	ccGenericPointCloud* otherCloud,
											CCLib::GenericProgressCallback* progressCb/*=nullptr*/,
											unsigned char octreeLevel/*=0*/)
{
if (!otherCloud || otherCloud->size() == 0)
{
	ccLog::Warning("[ccPointCloudImg::interpolateColorsFrom] Invalid/empty input cloud!");
	return false;
}
...
{
	ccLog::Warning("[ccPointCloudImg::interpolateColorsFrom] Clouds are too far from each other! Can't proceed.");
	return false;
}
...
if (!resizeTheRGBTable(false))
{
	ccLog::Warning("[ccPointCloudImg::interpolateColorsFrom] Not enough memory!");
	return false;
}
...
ccPointCloudImg* ccPointCloudImg::unrollOnCylinder(PointCoordinateType radius,
	...
	CCLib::GenericProgressCallback* progressCb/*=nullptr*/) const
{
	...
	//ccPointCloudImg* clone = const_cast<ccPointCloudImg*>(this)->cloneThis(0, true);
	//if (!clone)
	//{
	//	return 0;
	//}
	CCLib::ReferenceCloud duplicatedPoints(const_cast<ccPointCloudImg*>(this));
	...
	if (!center)
	{
		C = const_cast<ccPointCloudImg*>(this)->getOwnBB().getCenter();
		center = &C;
	}
	...
	ccPointCloudImg* clone = partialClone(&duplicatedPoints);
	if (clone)
	{
...
ccPointCloudImg* ccPointCloudImg::unrollOnCone(	double coneAngle_deg,
											const CCVector3& coneApex,
											unsigned char coneAxisDim,
											...
											CCLib::GenericProgressCallback* progressCb/*=nullptr*/) const
{
	...
	ccPointCloudImg* clone = const_cast<ccPointCloudImg*>(this)->cloneThis(nullptr, true);
	if (!clone)
	{
		return nullptr;
	}
...
//void ccPointCloudImg::unrollOnCone(PointCoordinateType baseRadius,
//	double alpha_deg,
...
int ccPointCloudImg::addScalarField(const char* uniqueName)
{
...
int ccPointCloudImg::addScalarField(ccScalarField* sf)
{
	assert(sf);
	...
	{
		ccLog::Warning(QString("[ccPointCloudImg::addScalarField] Name '%1' already exists!").arg(sf->getName()));
		return -1;
	}
	...
	if (sf->size() < m_points.size())
	{
		...
		{
			ccLog::Warning("[ccPointCloudImg::addScalarField] Not enough memory!");
			return -1;
		}
	}
...
bool ccPointCloudImg::toFile_MeOnly(QFile& out) const
{
...
bool ccPointCloudImg::fromFile_MeOnly(QFile& in, short dataVersion, int flags)
{
...
unsigned ccPointCloudImg::getUniqueIDForDisplay() const
{
...
CCLib::ReferenceCloud* ccPointCloudImg::crop(const ccBBox& box, bool inside/*=true*/)
{
	if (!box.isValid())
	{
		ccLog::Warning("[ccPointCloudImg::crop] Invalid bounding-box");
		return nullptr;
	}
	...
	if (count == 0)
	{
		ccLog::Warning("[ccPointCloudImg::crop] Cloud is empty!");
		return nullptr;
	}
	...
	{
		ccLog::Warning("[ccPointCloudImg::crop] Not enough memory!");
		delete ref;
		return nullptr;
	}
...
CCLib::ReferenceCloud* ccPointCloudImg::crop2D(const ccPolyline* poly, unsigned char orthoDim, bool inside/*=true*/)
{
	if (!poly)
	{
		ccLog::Warning("[ccPointCloudImg::crop2D] Invalid input polyline");
		return nullptr;
	}
	if (orthoDim > 2)
	{
...
bool ccPointCloudImg::updateVBOs(const CC_DRAW_CONTEXT& context, const glDrawParams& glParams)
{
	if (isColorOverriden())
	{
	...
	{
		//ccLog::Warning(QString("[ccPointCloudImg::updateVBOs] VBOs are in a 'failed' state... we won't try to update them! (cloud '%1')").arg(getName()));
		return false;
	}

	if (!m_currentDisplay)
	{
		ccLog::Warning(QString("[ccPointCloudImg::updateVBOs] Need an associated GL context! (cloud '%1')").arg(getName()));
		assert(false);
		return false;
	}
	...
	if (m_vboManager.vbos.size() != chunksCount)
	{
		...
		{
			ccLog::Warning(QString("[ccPointCloudImg::updateVBOs] Not enough memory! (cloud '%1')").arg(getName()));
			m_vboManager.state = vboSet::FAILED;
			return false;
		}
	}
	...
	m_vboManager.totalMemSizeBytes = 0;
	{
		...
		for (size_t i = 0; i < chunksCount; ++i)
		{
			...
			if (glFunc)
			{
				CatchGLErrors(glFunc->glGetError(), "ccPointCloudImg::vbo.init");
			}
			...
			if (vboSizeBytes > 0)
			{
				...
				QOpenGLFunctions_2_1* glFunc = context.glFunctions<QOpenGLFunctions_2_1>();
				assert(glFunc != nullptr);
				if (CatchGLErrors(glFunc->glGetError(), "ccPointCloudImg::updateVBOs"))
				{
					vboSizeBytes = -1;
				}
				else
			...
			if (vboSizeBytes < 0) //VBO initialization failed
			{
				...
				if (i == 0)
				{
					ccLog::Warning(QString("[ccPointCloudImg::updateVBOs] Failed to initialize VBOs (not enough memory?) (cloud '%1')").arg(getName()));
					m_vboManager.state = vboSet::FAILED;
...
int ccPointCloudImg::VBO::init(int count, bool withColors, bool withNormals, bool* reallocated/*=0*/)
{
	//required memory
	...
	if (!bind())
	{
		ccLog::Warning("[ccPointCloudImg::VBO::init] Failed to bind VBO to active context!");
		destroy();
		return -1;
	}
	...
	if (totalSizeBytes != size())
	{
		...
		{
			ccLog::Warning("[ccPointCloudImg::VBO::init] Not enough (GPU) memory!");
			release();
			destroy();
			return -1;
		}
...
void ccPointCloudImg::releaseVBOs()
{
...
void ccPointCloudImg::removeFromDisplay(const ccGenericGLDisplay* win)
{
...
bool ccPointCloudImg::computeNormalsWithGrids(	double minTriangleAngle_deg/*=1.0*/,
...
bool ccPointCloudImg::orientNormalsWithGrids(ccProgressDialog* pDlg/*=0*/)
{
	unsigned pointCount = size();
	if (pointCount == 0)
	{
	...
	for (size_t gi = 0; gi < gridCount(); ++gi)
	{
		const ccPointCloudImg::Grid::Shared& scanGrid = grid(gi);
		if (scanGrid && scanGrid->indexes.empty())
		{
			//empty grid, we skip it
			continue;
...
bool ccPointCloudImg::orientNormalsTowardViewPoint( CCVector3 & VP, ccProgressDialog* pDlg)
{
	int progressIndex = 0;
...
bool ccPointCloudImg::computeNormalsWithOctree(CC_LOCAL_MODEL_TYPES model,
...
//bool ccPointCloudImg::orientNormalsWithMST(unsigned kNN/*=6*/,
//										ccProgressDialog* pDlg/*=0*/)
//{
//	return ccMinimumSpanningTreeForNormsDirection::OrientNormals(this, kNN, pDlg);
//}

//bool ccPointCloudImg::orientNormalsWithFM(	unsigned char level,
//										ccProgressDialog* pDlg/*=0*/)
//{
//	return ccFastMarchingForNormsDirection::OrientNormals(this, level, pDlg);
//}

bool ccPointCloudImg::hasSensor() const
{
...
unsigned char ccPointCloudImg::testVisibility(const CCVector3& P) const
{
...
void ccPointCloudImg::clearFWFData()
{
	m_fwfWaveforms.resize(0);
	m_fwfDescriptors.clear();
}

bool ccPointCloudImg::computeFWFAmplitude(double& minVal, double& maxVal, ccProgressDialog* pDlg/*=0*/) const
{
	minVal = maxVal = 0;
...
bool ccPointCloudImg::enhanceRGBWithIntensitySF(int sfIdx, bool useCustomIntensityRange/*=false*/, double minI/*=0.0*/, double maxI/*=1.0*/)
{
	...
	if (intRange < 1.0e-6)
	{
		ccLog::Warning("[ccPointCloudImg::enhanceRGBWithIntensitySF] Intensity range is too small");
		return false;
	}
...
ccMesh* ccPointCloudImg::triangulateGrid(const Grid& grid, double minTriangleAngle_deg/*=0.0*/) const
{
	//the code below has been kindly provided by Romain Janvier
	CCVector3 sensorOrigin = CCVector3::fromArray((grid.sensorPosition.getTranslationAsVec3D()/* + m_globalShift*/).u);

	ccMesh* mesh = new ccMesh(const_cast<ccPointCloudImg*>(this));
	mesh->setName("Grid mesh");
	if (!mesh->reserve(grid.h * grid.w * 2))
	{
		ccLog::Warning("[ccPointCloudImg::triangulateGrid] Not enough memory");
		return nullptr;
	}
...
bool ccPointCloudImg::exportCoordToSF(bool exportDims[3])
{
	...
	for (unsigned d = 0; d < 3; ++d)
	{
		...
		if (sfIndex < 0)
		{
			ccLog::Warning("[ccPointCloudImg::exportCoordToSF] Not enough memory!");
			return false;
		}
